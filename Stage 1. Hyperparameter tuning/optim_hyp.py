...
def objective(config):
    ...
    if not MEASURE_TIME:  
        ...
        if DIST:
            dist_util.broadcast(config)
            print(f'Rank {dist_util.get_rank()} (master) got config {config}')
    ...
    for pr_auc in train_dist.main(config,yeild_prauc = True):
        if earlyStopper(pr_auc):
            if DIST:
                dist_util.broadcast("stop")
                print(f'early stop')
            break
        else:
            if DIST:
                dist_util.broadcast("continue")
...
def main():
    ...
    if dist_util.is_master():
        ...
        while True:
            config = dist_util.broadcast()
            #print(f'Rank {dist_util.get_rank()} got config {config}')
            for _ in train_dist.main(config,yeild_prauc = True):
                if dist_util.broadcast() == "stop":
                    break
