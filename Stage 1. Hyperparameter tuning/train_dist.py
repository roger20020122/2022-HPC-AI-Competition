...
from models import swin_transformer, leopard
import dataset
import metrics
...
def log(*msg):
    if dist_util.is_master() or not args.dist:
        print(*msg)
...
def main(arg_overrides={},yeild_prauc = False):
    ...
    is_master = not args.dist or dist_util.is_master()
    ...
    if args.dist:
        ...
        print(f'Process start using device {dist_util.dev()} on machine {dist_util.get_hostname()}')
    ...
    if args.model == 'swin':
        ...
        model = leopard.LeopardUnet(1,ds.feature.shape[-1],dropout=args.dropout,num_blocks=args.num_blocks,initial_filter=args.initial_filter,size_kernel=args.kernel_size,scale_filter=args.scale_filter)
    else:
    ...
    log(1.5)
    if args.dist:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[dist_util.get_local_rank()], output_device=dist_util.get_local_rank())
    log(2)
...
def run_val(dl,model):
    ...
    for name, m in mets.items():
        ...
        results[name] = m(preds,ys).to(dist_util.dev())
    return results
