...
def get_args():
    ...
    parser.add_argument('-f', '--path', default='/scratch/rw67/', type=str,
...
def main():
    ...
    tik = time.time()
    model.fit(x=train_dataset, epochs=190, batch_size=batch_size, validation_data=val_dataset, callbacks=callbacks)
    tok = time.time()
